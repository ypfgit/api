$(document).ready(
    function() {
        ...
        var treeCombo = new TreeCombo("parent_name", "parent_id", "J_treeCombo", "J_columnTree", r_path + "/column/getTreeList");
        var setting = {
            view: {
            ...
            async: {
                url: r_path + '/column/getTreeNodes',
                enable: true,
                autoParam: ["id"]
        ...
        var zTree = $.fn.zTree.init($("#J_tree"), setting, [{ name: "全部分类", id: "01", parent_id: "0", isParent: true, open: true }]);
        var nodes = zTree.getNodes();
        if (nodes.length > 0) {
        ...
        $table.on('click', ".btn-edit", function() {
            ...
            layer.confirm('确认要删除吗？',{icon: 3, title:'提示'}, function(index) {
            	$.post(r_path + '/column/delete', { ids: item.id },
                        function() {
                            layer.msg('已删除!', {
                                icon: 1,
                                time: 1000
        ...
        $("#J_form").validate({
            ...
            submitHandler: function(form) {
                var url = '';
                if(VM.mode=='add'){
                    url = r_path + '/column/save';
                }else if(VM.mode=='edit'){
                    url = r_path + '/column/edit';
                }
                $(form).ajaxSubmit({
                    url: url,
                    type: "post",
