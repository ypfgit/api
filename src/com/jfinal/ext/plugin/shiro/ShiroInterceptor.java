...
import com.plat.core.shiro.CaptchaUsernamePasswordToken;
		...
		if (ah != null) {
			try {
				...
				if (!subject.isAuthenticated() && subject.isRemembered()) {
					User user = User.dao.findByAccount(subject.getPrincipal()
							.toString());
					subject.login(new CaptchaUsernamePasswordToken(user
							.getStr("login_name"), user.getStr("password"), true, ai
							.getController().getRequest().getRemoteAddr(),"remeber"));

					Session session = subject.getSession(false);
					session.setAttribute(TSConstants.S_USERINFO, user);
				}
				...
				if (StrKit.notBlank(ShiroKit.getLoginUrl())) {
					...
					if (ai.getController().getRequest().getMethod()
							.equalsIgnoreCase("GET")) {
						ai.getController().setSessionAttr(
								ShiroKit.getSavedRequestKey(),
								ai.getActionKey());
					}
