...
public class TSUtils {
	public static Log log = Log.getLog(TSUtils.class);
	...
	public static boolean isTheStyle(String pStr, String pStyle) {
		for (int i = 0; i < pStr.length(); i++) {
			...
			if (pStyle.equals(TSConstants.S_STYLE_N)) {
				if (!Character.isDigit(c))
					return false;
			} else if (pStyle.equals(TSConstants.S_STYLE_L)) {
				if (!Character.isLetter(c))
					return false;
			} else if (pStyle.equals(TSConstants.S_STYLE_NL)) {
	...
	public static java.sql.Timestamp stringToSqlDate(String strDate) {
		Date date = stringToDate(strDate,
				"yyyy-MM-dd HH:mm:ss",
				"yyyy/MM/dd HH:mm:ss");
		java.sql.Timestamp dd = new java.sql.Timestamp(date.getTime());
	...
	public static String getGBK(String pString){
		...
		try {
			...
			log.error(TSConstants.Exception_Head + "不支持的字符集编码");
			e.printStackTrace();
		}
		return pString;
	}
